...
timings.sort(key=lambda x: x[0][1])

z3_times_with_opt = [x[0][1] for x in timings]
z3_times_without_opt = [x[1][1] for x in timings]

# Make the dots small
plt.scatter(range(len(timings)), z3_times_with_opt, label="With optimization", s=1)
plt.scatter(
    range(len(timings)), z3_times_without_opt, label="Without optimization", s=1
)
